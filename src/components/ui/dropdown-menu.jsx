...
import { Check, CheckCheck, CheckIcon, ChevronRight, CircleCheck } from "lucide-react";
...
const DropdownMenuItem = React.forwardRef(
  ({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex cursor-default select-none items-center justify-between rounded-sm px-2.5 py-1.5 text-xs outline-none transition-colors focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        inset && "pl-8",
        className
      )}
      {...props}
  ...
  ({ className, children, ...props }, ref) => (
    ...
    >
      ...
      <span className="absolute right-2 flex items-center justify-center">
        <DropdownMenuPrimitive.ItemIndicator>
          {
            props?.icon === 'CircleCheck' ?
          <CircleCheck className="h-5 w-5 fill-lightBlack stroke-white" />
          :
